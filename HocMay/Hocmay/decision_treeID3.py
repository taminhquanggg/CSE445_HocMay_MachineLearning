...
class DecisionTreeID3(object):# xây dựng cây
    ...
    def fit(self, data, target):
        ...
        #lựa chọn taaph thuộc tính tốt nhất treenode để làm root 
        self.root = TreeNode(ids = ids, entropy = self._entropy(ids), depth = 0)
        queue = [self.root]
        while queue:# xây dựng nút con từ nút gốc(48-56)
            node = queue.pop()
    ...
    def predict(self, new_data):#duyệt cây để dự đoán
        """
...
if __name__ == "__main__":
    df = pd.read_csv('bai1.csv',index_col = 0, parse_dates = True)
    X = df.iloc[:, :-1]
    y = df.iloc[:, -1]
    tree = DecisionTreeID3(max_depth = 4, min_samples_split = 1)
    tree.fit(X,y)
